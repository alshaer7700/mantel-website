...
export function MenuItemRow({ item }: { item: MenuItem }) {
  return (
    <div className="py-5 flex justify-between items-start gap-6 border-b border-foreground/60">
      <div>
        {/* Underline sits on an inline-block so it spans the name only, not the
            full column. Same rule as the FAQ and the row divider: 1px solid
            foreground/60. */}
        <p className="font-mono font-medium text-[15px]">
          <span className="inline-block border-b border-foreground/60 pb-[3px]">
            {item.name}
          </span>
        </p>
