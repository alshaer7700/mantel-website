...
- [ ] **Finish the Resend setup — no notification email is being delivered.** FormSubmit is gone; contact messages, orders and subscriptions now go out through three Edge Functions via Resend, and neither half of that path is live yet. Two things are missing, and both are owner clicks: the three DNS records Resend is waiting on (none of them exist in the `bymantel.com` zone — the domain has sat at "Missing records" since it was added), and `RESEND_API_KEY` in the Supabase Edge Function secrets. Every submission is still saved to the database, so nothing is being lost; nobody is being told about it. Records, order of operations and how to verify: `guidelines/EMAIL.md`.
